...
impl From<RpcLightClientProofError> for crate::errors::RpcError {
    fn from(error: RpcLightClientProofError) -> Self {
        let error_data = match &error {
            RpcLightClientProofError::UnknownBlock { error_message } => Some(Value::String(
                format!("DB Not Found Error: {}", error_message),
            )),
        ...
        let error_data = match serde_json::to_value(error) {
            ...
            Err(err) => {
                return Self::new_internal_error(
                    None,
                    format!(
                        "Failed to serialize RpcLightClientNextBlockError: {:?}",
                        err
                    ),
                )
            }
